...
function buildVisualProfile() {
    ...
    const spectrumFps = manualFps || 24;
    const pixelRatio = 1;
    return {
        lite: true,
        pixelRatio,
        spectrumPixelRatio: Math.min(0.75, dpr),
        spectrumFps,
...
export function drawSpectrum() {
    ...
    state.smoothDisplayBins = easeBins(rawBins, state.smoothDisplayBins, 0.94, 0.24);
    state.smoothDisplayPeaks = easeBins(rawPeaks, state.smoothDisplayPeaks, 0.97, 0.05);
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        spectrumContext.fillStyle = paints.barSolid;
        ...
        const capHeight = Math.max(2, height * 0.014);
    ...
    if (count >= 2) {
        ...
        spectrumContext.lineWidth = 2.1 + state.specRms * 1.55;
        spectrumContext.lineJoin = 'round';
        spectrumContext.lineCap = 'round';
        spectrumContext.stroke();
    }

    spectrumContext.fillStyle = paints.baseSolid;
